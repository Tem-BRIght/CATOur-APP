...
export function subscribeRecommendedDestinations(
  ...
  const unsub = onSnapshot(query(col(), limit(100)), (snap) => {
    const data = fromSnapshot(snap).filter((item) => (item as any).status !== 'draft')
      .sort((a, b) => Number(b.recommended) - Number(a.recommended) || (b.rating || 0) - (a.rating || 0));
    cacheDestinationsList(data);
    onChange(data);
  }, (error) => {
    console.warn('[destinationService] recommended subscription failed:', error);
    if (cached) onChange(cached.data);
  });
...
export function subscribePopularDestinations(
  ...
  const unsub = onSnapshot(query(col(), limit(100)), (snap) => {
    ...
    console.warn('[destinationService] popular subscription failed:', error);
    if (cached) onChange(cached.data);
  });
